 ...
 *
 * 这些API有意提供非常弱的一致性语义。 这些API的使用者应准备处理空的/丢失的信息。
 * 例如，一个工作的阶段ID可能是已知的，但是状态API可能没有关于那些阶段的详细信息的任何信息，因此getStageInfo可能会为有效的阶段ID返回None。
 *
 * 为了限制内存使用，这些API仅提供有关最近作业/阶段的信息。 这些API将为最后的“ spark.ui.retainedStages”阶段和“ spark.ui.retainedJobs”作业提供信息。
 *
